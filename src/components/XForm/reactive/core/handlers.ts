...
import { Callback, Key, Operation, Reaction } from './types'
import {
  isIterateFunction,
  isObject,
  isEffectFunction,
...
function registerReactiveFunction(f: Function, operation: Operation) {
  ...
  unobserve(listernerMap.get(key) as any)

  listernerMap.set(
    key,
    observe(operation => {
      Reflect.set(reaction as any, key, copy(reaction, operation))
    })
  )
...
const baseHandlers = {
  get(target, key, receiver) {
    ...
    if (isFactoryFunction(result)) return result({ target, key, receiver })
    ...
    if (!rawToParent.has(result))
      rawToParent.set(result, rawToReaction.get(target) as any)
  ...
  set(target, key, value) {
    ...
    if (isObject(oldValue) && isObject(value))
      rawToCallbackMap.set(target[key], rawToCallbackMap.get(oldValue) as any)
...
export function combine(source: Reaction, auxiliary: Reaction) {
  ...
  const raw = reactionToRaw.get(source) as any
